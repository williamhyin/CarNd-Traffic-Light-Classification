...
flags.DEFINE_float('split_train_test', 0, 'If supplied specifies the amount of samples to use for evaluation')
...
def start_create_record(dataset,annotations_dir,examples_list,label_map_dict,output_path):
    writer = tf.python_io.TFRecordWriter(output_path)
    for example in tqdm(examples_list, desc='Converting', unit=' images'):
        path = os.path.join(dataset, example + '.jpg')
        example_path = os.path.join(annotations_dir, example + '.xml')
        if os.path.isfile(path):

            with tf.gfile.GFile(example_path, 'r') as fid:
                xml_str = fid.read()
            ...
            data['filename'] = os.path.join(os.path.abspath(
                dataset), os.path.basename((data['filename'])).replace("png","jpg"))

            data['filename'] = create_jpg_imgs(data['filename'])
...
def main(_):
    ...
    if dataset.split('.')[-1] == r'yaml':
        ...
        examples_list = [os.path.splitext(name)[0] for name in os.listdir(
            annotations_dir) if os.path.isfile(os.path.join(annotations_dir, name))]

        if split_train_test !=0:
            label_files_train, label_files_eval = train_test_split(examples_list, test_size=split_train_test,
                                                                   shuffle=True)
            output_train_path=os.path.splitext(output_path)[0]+"_train.record"
            output_eval_path = os.path.splitext(output_path)[0] + "_eval.record"
            print('TF record file for training created with {} samples: {}'.format(len(label_files_train),
                                                                                     output_train_path))
            start_create_record(dataset,annotations_dir, label_files_train, label_map_dict, output_train_path)

            print('TF record file for validation created with {} samples: {}'.format(len(label_files_eval),
                                                                                  output_eval_path))
            start_create_record(dataset, annotations_dir,label_files_eval, label_map_dict, output_eval_path)
        else:
            print('TF record file for training created with {} samples: {}'.format(len(examples_list),
                                                                                   output_path))
            start_create_record(dataset, annotations_dir,examples_list, label_map_dict, output_path)


    print("finish record generation")
